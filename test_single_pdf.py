"""
test_single_pdf.py — v6 (Google Cloud Vision API)
===================================================
Architecture: Page-level Vision API (1 API call per page, NOT 1 per card)
  - 2 API calls for this 2-page test vs 120+ Tesseract calls
  - For 371 PDFs × 30 pages = ~11,130 total API calls (~$16 cost)
  - Expected accuracy: EPIC 90-95%, other fields 98-99%
...
TARGET_PDF     = os.path.join(SCRIPT_DIR,
                 "2026-FC-EROLLGEN-S22-119-SIR-FinalRoll-Revision2-ENG-9-WI.pdf")
RENDER_SCALE   = 2          # 2× is fine for Vision API (it handles lower DPI well)
MAX_TEST_PAGES = 2
TEST_CSV       = os.path.join(SCRIPT_DIR, "test_single_result.csv")
...
def main():
    print("=" * 65)
    print("  ELECTION ROLL OCR — v6 (Google Cloud Vision API)")
    print("=" * 65)

    if not os.path.exists(TARGET_PDF):
        print(f"[ERROR] PDF not found: {TARGET_PDF}")
        return
    ...
    print(f"[PDF]  {os.path.basename(TARGET_PDF)}")
    ...
    print(f"       = {end-start} Vision API calls total\n")
    ...
    for page_num in range(start, end):
        ...
        for idx, (card_img, (cx1,cy1,cx2,cy2)) in enumerate(zip(cards, coords)):
            ...
            print(f"\n  Card {idx+1:>2}  (#{grand_total:>4})  {ok}{flag_str}")
            print(f"    Serial   : {serial}")
            print(f"    EPIC ID  : {epic}")
            print(f"    Name     : {fields['name']}")
            print(f"    Relation : {fields['relation_type']} -> {fields['relation_name']}")
            print(f"    House #  : {fields['house_number']}")
            print(f"    Age      : {fields['age']}")
            print(f"    Gender   : {fields['gender']}")
            print(f"    [VISION] : {ctext[:200]}")
            ...
            all_records.append(dict(
                ...
                source_file   = os.path.basename(TARGET_PDF),
